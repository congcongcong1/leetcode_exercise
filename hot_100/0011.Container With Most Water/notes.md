# [11]. [盛最多水的容器]

> 难度: [Medium] | 分类: [数组/链表/...] | 日期: 2026-3-6

## 题目摘要

给定一个长度为 n 的整数数组 height 。有 n 条垂线，第 i 条线的两个端点是 (i, 0) 和 (i, height[i]) 。
找出其中的两条线，使得它们与 x 轴共同构成的容器可以容纳最多的水。
返回容器可以储存的最大水量。
说明：你不能倾斜容器。

## 解题思路

### 第一反应
- 看到题目后最直觉的想法是什么？
  如果已经确定了一对指针left和right，此时他们维护的面积最大，此时要移动，如果不移动短板，那么再怎么往中间移动移动水都不会比上次多，所以短板应该向右边移动
- 为什么这个想法行得通 / 行不通？
  行得通，因为排序后可以使用双指针法，然后根据短板效应移动短板

### 最终解法
- 核心思路：
  1. 用了缩减搜索空间的思想，每排除一根柱子那么就可以排除一行以及一列的搜索空间
  2. 如果已经确定了一对指针left和right，此时他们维护的面积最大，此时要移动，如果不移动短板，那么再怎么往中间移动水都不会比上次多，所以短板应该向右边移动


### 复杂度分析
| 解法 | 时间复杂度 | 空间复杂度 |
|------|-----------|-----------|
| 最优 | O(n) | O(1) |
时间复杂度是O(n)是因为用了缩减搜索空间的思想，每排除一根柱子那么就可以排除一行以及一列的搜索空间

## 关键知识点

- [ ] [双指针无需排序]
- [ ] [短板效应有限考虑短板]

## 踩坑记录

- ⚠️ [短板效应未考虑]

## 相关题目

| 题号 | 题目 | 关系 |
|------|------|------|
| [167](https://leetcode.cn/problems/two-sum-ii-input-array-is-sorted/) | [两数之和 II - 输入有序数组](https://leetcode.cn/problems/two-sum-ii-input-array-is-sorted/) | 类似题 |
| [18](https://leetcode.cn/problems/4sum/) | [四数之和](https://leetcode.cn/problems/4sum/) | 进阶题 |

## 总结

[短板效应以及双指针的移动，以及双指针可以缩短搜索空间]
